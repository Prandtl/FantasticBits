...
class Player
{
    static void Main(string[] args)
    {
        ...
        while (true)
        {
            ...
            master.DoSomething(snaffles, myTeamId);
            slave.DoSomething(snaffles, myTeamId);
        }
    }
}
...
public class Snaffle: GameObject{}
...
public class Forwarder: Wizard
{
    ...
    public Snaffle Target {get;set;}
    public bool IsMasterPlayer {get;set;}
    public Forwarder Companion {get;set;}


    public void DoSomething(List<Snaffle> snaffles, int teamId)
    { 
        ...
        if(Game.MyMagic > Rules.FlipendoCost)
        {
            ...
            var possibleFlipendo = snaffles
                .Where(s => Utils.IsInsideTriangle(s.X, s.Y, X, Y, enemyGoalX, Rules.GoalBottom - 500, enemyGoalX, Rules.GoalTop + 500));
            ...
            else
            {
                ...
                if(orderedForFirst[0] == orderedForSecond[0])
                {
                    var dist = Utils.GetDistance(X, Y, orderedForFirst[0].X, orderedForFirst[0].Y) +
                        Utils.GetDistance(Companion.X, Companion.Y, orderedForSecond[1].X, orderedForSecond[1].Y);
                        
                    var swapDist = Utils.GetDistance(X, Y, orderedForFirst[1].X, orderedForFirst[1].Y) +
                        Utils.GetDistance(Companion.X, Companion.Y, orderedForSecond[0].X, orderedForSecond[0].Y);
